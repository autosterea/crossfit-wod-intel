...
function ComplexityMeter({ value }: { value: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-2 rounded-full bg-[#1e1e3a] overflow-hidden">
        <div className="h-full rounded-full transition-all duration-500" style={{ width: `${pct}%`, background: color }} />
      </div>
      <span className="text-xs font-mono text-slate-300 w-10 text-right">{value.toFixed(1)}/5</span>
    </div>
  )
}
    ...
    <div className="space-y-4">
      ...
      <div className="relative">
        <input
          ...
          className="w-full bg-[#12121a] border border-[#1e1e3a] rounded-lg px-4 py-3 text-sm text-white placeholder-slate-500 focus:border-blue-500/50 focus:outline-none"
        />
        ...
        {showDropdown && searchResults.length > 0 && (
          <div
            ref={dropdownRef}
            className="absolute z-50 mt-1 w-full max-h-80 overflow-y-auto bg-[#12121a] border border-[#2a2a5a] rounded-lg shadow-2xl"
          >
            {searchResults.map((w, i) => (
              <button
                key={`${w.d}-${i}`}
                onClick={() => handleSelect(w)}
                className="w-full text-left px-4 py-2.5 hover:bg-[#1e1e3a] transition-colors border-b border-[#1e1e3a] last:border-0 flex items-start gap-3"
              >
                <span className="text-[10px] font-mono text-slate-500 whitespace-nowrap pt-0.5">{w.d}</span>
        ...
        {showDropdown && query.trim() && searchResults.length === 0 && (
          <div className="absolute z-50 mt-1 w-full bg-[#12121a] border border-[#2a2a5a] rounded-lg p-4 text-sm text-slate-500">
      ...
      {/* No selection state */}
      {!selected && (
        <div className="bg-[#12121a] rounded-xl border border-[#1e1e3a] p-12 text-center">
          <div className="text-4xl mb-4 opacity-30">&#x1F50D;</div>
          <div className="text-slate-400 text-sm">Search for a workout above to decode it</div>
      ...
      {selected && analysis && (
        <>
          {/* Workout header */}
          <div className="bg-gradient-to-r from-blue-500/5 via-purple-500/5 to-rose-500/5 rounded-xl border border-[#2a2a5a] p-6">
            <div className="flex items-start justify-between mb-3">
              <div>
                <div className="text-[10px] font-mono text-slate-500 mb-1">{formatDate(selected.d)}</div>
                {selected.nw && (
              ...
              <button
                ...
                className="text-slate-500 hover:text-white transition-colors text-lg leading-none px-2"
                title="Clear selection"
              >
                &times;
              </button>
            </div>
            <div className="bg-[#0a0a12] rounded-lg p-4 mt-3 border border-[#1e1e3a]">
              <pre className="text-xs text-slate-300 whitespace-pre-wrap font-mono leading-relaxed">{selected.s}</pre>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-[#12121a] rounded-xl p-5 border border-[#1e1e3a]">
                <h3 className="text-xs font-medium text-slate-400 mb-1">Physical Skills Targeted</h3>
                <p className="text-[10px] text-slate-500 mb-3">CrossFit's 10 General Physical Skills developed by this workout</p>
              ...
              {/* Skills list */}
              <div className="bg-[#12121a] rounded-xl p-5 border border-[#1e1e3a]">
                <h3 className="text-xs font-medium text-slate-400 mb-3">Skill Emphasis Breakdown</h3>
          ...
          <div className="bg-[#12121a] rounded-xl p-5 border border-[#1e1e3a]">
            ...
            {analysis.similar.length === 0 ? (
              <div className="text-xs text-slate-500 py-4 text-center">No similar workouts found.</div>
            ) : (
                ...
                {analysis.similar.map((s, i) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full text-left bg-[#0a0a12] hover:bg-[#161622] rounded-lg p-4 border border-[#1e1e3a] hover:border-[#2a2a5a] transition-colors group"
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-[10px] font-mono text-slate-500">{sw.d}</span>
                            ...
                            <span className="text-[9px] text-slate-500">{sw.td}</span>
                            <span className="text-[9px] text-slate-500">{sw.st}</span>
                          </div>
