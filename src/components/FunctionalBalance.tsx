...
function RatioGauge({ label, ratio, leftLabel, rightLabel, leftColor, rightColor }: {
  ...
  return (
    <div className="bg-[#12121a] rounded-xl p-5 border border-[#1e1e3a]">
      ...
      <div className="relative h-6 rounded-full overflow-hidden bg-[#1e1e3a]">
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-gradient-to-br from-blue-500/10 to-purple-500/10 rounded-xl p-5 border border-blue-500/20">
          <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Overall Balance</div>
          <div className="text-3xl font-bold font-mono text-blue-400">
            {((1 - analysis.functionalBalance) * 100).toFixed(0)}%
          </div>
          <div className="text-[10px] text-slate-400 mt-1">100% = perfect distribution</div>
        </div>
        <div className="bg-[#12121a] rounded-xl p-5 border border-[#1e1e3a]">
          <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Push Movements</div>
          <div className="text-2xl font-bold font-mono text-rose-400">{pushTotal.toLocaleString()}</div>
          <div className="text-[10px] text-slate-500">{((pushTotal / data.overview.total_workouts) * 100).toFixed(1)}% of WODs</div>
        </div>
        <div className="bg-[#12121a] rounded-xl p-5 border border-[#1e1e3a]">
          <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Pull Movements</div>
          <div className="text-2xl font-bold font-mono text-emerald-400">{pullTotal.toLocaleString()}</div>
          <div className="text-[10px] text-slate-500">{((pullTotal / data.overview.total_workouts) * 100).toFixed(1)}% of WODs</div>
        </div>
        <div className="bg-[#12121a] rounded-xl p-5 border border-[#1e1e3a]">
          <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Olympic Lifts</div>
          <div className="text-2xl font-bold font-mono text-purple-400">{olyTotal.toLocaleString()}</div>
          <div className="text-[10px] text-slate-500">{((olyTotal / data.overview.total_workouts) * 100).toFixed(1)}% of WODs</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        {/* Radar */}
        <div className="bg-[#12121a] rounded-xl p-5 border border-[#1e1e3a]">
          <h3 className="text-xs font-medium text-slate-400 mb-3">Movement Pattern Radar</h3>
          <div style={{width:"100%",height:380}}><ResponsiveContainer width="100%" height="100%">
            <RadarChart data={radarData}>
              <PolarGrid stroke="#1e1e3a" />
              <PolarAngleAxis dataKey="pattern" tick={{ fontSize: 8, fill: '#94a3b8' }} />
        ...
        {/* Bar ranking */}
        <div className="bg-[#12121a] rounded-xl p-5 border border-[#1e1e3a]">
          <h3 className="text-xs font-medium text-slate-400 mb-3">Pattern Frequency (% of WODs)</h3>
          <div style={{width:"100%",height:380}}><ResponsiveContainer width="100%" height="100%">
            <BarChart data={barData} layout="vertical" margin={{ left: 110 }}>
              ...
              <YAxis type="category" dataKey="name" tick={{ fontSize: 9, fill: '#94a3b8' }} width={105} />
              <Tooltip contentStyle={{ background: '#1e1e3a', border: '1px solid #2a2a5a', borderRadius: 8, fontSize: 12 }} formatter={(v: any) => [`${v}%`]} />
      ...
      <div className="bg-[#12121a] rounded-xl p-5 border border-[#1e1e3a]">
        <h3 className="text-xs font-medium text-slate-400 mb-3">Muscle Group Coverage (% of WODs targeting each group)</h3>
      ...
      {/* Patterns over time */}
      <div className="bg-[#12121a] rounded-xl p-5 border border-[#1e1e3a]">
        <h3 className="text-xs font-medium text-slate-400 mb-3">Functional Patterns Over Time (WODs per year)</h3>
        ...
        <div className="flex flex-wrap gap-2 mt-3 justify-center">
          {Object.entries(FUNCTIONAL_PATTERN_COLORS).map(([key, color]) => (
            <span key={key} className="flex items-center gap-1 text-[8px] text-slate-500">
